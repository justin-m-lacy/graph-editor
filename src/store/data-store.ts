...
import { mergeValues, ResetIds } from "@/util/data";
...
export const useDataStore = defineStore('save', () => {
	...
	return {
		...
		mergeFrom(values: AppData) {

			mergeValues(values.points, data.value.points);
			mergeValues(values.clusters, data.value.clusters);
